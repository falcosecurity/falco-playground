...
function __callback_getm_to_buffer(url, _res, _rej, dumpto) {
    ...
    https.get(url, (res) => {
        if (res.statusCode === 301 || res.statusCode === 302) {
            ...
            return __callback_getm_to_buffer(res.headers.location, _res, _rej, dumpto);
        }
        ...
        res.on("data", (chunk) => {
            if (dumpto)
                dumpto.write(Buffer.from(chunk));
            else
...
let zipstream = fs.createWriteStream(tgzname);
...
if (!fs.existsSync(exrdirc))
    await unzip(tgzname);


// copying files
console.log("Copying files");
fs.copyFile(exrdirc + "/usr/bin/falco.wasm", "public/falco.wasm", (e) => { if (e) console.error(e); });
fs.copyFile(exrdirc + "/usr/bin/falco.js", "src/Hooks/falco.js", (e) => { if (e) console.error(e); });
